...
from .rag_model.rag_pipeline import  preprocess_query
from django.conf import settings
@csrf_exempt # API 테스트를 위해 CSRF 보호 기능을 일시적으로 해제 (운영 환경에서는 DRF 권장)
def chatbot_recommendation_api(request):
    qa_chain = settings.RAG_CHAIN_STORE.get('qa_chain')
    # 1. POST 요청 확인
    ...
    #     return JsonResponse({"error": "챗봇 시스템이 아직 준비되지 않았습니다."}, status=503)
    if qa_chain is None:
        # 챗봇 초기화가 실패했을 때 503 반환
        return JsonResponse({"error": "챗봇 시스템이 아직 준비되지 않았습니다. (초기화 실패)"}, status=503)
    try:
        # 3. JSON 데이터 파싱
        ...
        if not user_query:
            return JsonResponse({"error": "요청 본문에 'query' 필드가 필요합니다."}, status=400)

        # 4. 쿼리 전처리 및 RAG 실행
        preprocessed_query = preprocess_query(user_query) # rag_service.py에서 import
        # answer = global_qa_chain.run(user_query)
        answer = qa_chain.run(user_query)
        # .run 메서드 실행 (동기 실행)
        # answer = global_qa_chain.run(preprocessed_query)

        # 5. JSON 응답 반환
        return JsonResponse({"query": user_query, "recommendation": answer})

    except Exception as e:
        return JsonResponse({"error": f"서버 처리 중 오류 발생: {str(e)}"}, status=500)
